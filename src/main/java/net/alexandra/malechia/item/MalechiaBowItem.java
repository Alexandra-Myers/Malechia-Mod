...
public class MalechiaBowItem extends RangedWeaponItem {
    ...
    public void onStoppedUsing(ItemStack stack, World world, LivingEntity user, int remainingUseTicks) {
        if (user instanceof PlayerEntity) {
            ...
            boolean bl = (itemStack.getItem() instanceof ArrowItem && !itemStack.isEmpty());
            if (bl) {
                ...
                if (!((double)f < 0.1D)) {
                    ...
                    if (!world.isClient) {
                        ...
                        persistentProjectileEntity.setVelocity(playerEntity, playerEntity.getPitch(), playerEntity.getYaw(), 0.0F, f * 10.0F, 0.0F);
                        if (f == 1.0F) {
                        ...
                        if (j > 0) {
                            persistentProjectileEntity.setDamage(persistentProjectileEntity.getDamage() + (double)j * 0.375D + 0.5D);
                        }
